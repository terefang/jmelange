...
public class BsdNtCrypt
{
	public static final String MSNT_PREFIX = "{MSNT}";
	public static final String NT_PREFIX = "{NT}";
	public static final String NT2_PREFIX = "$NT$";
	public static final String BSDNT_PREFIX = "$3$$";

	public static final boolean checkPassword(String givenPasswaord, String encryptedPassword)
	{
		if(encryptedPassword.startsWith("{MSNT}"))
		{
			return encryptedPassword.substring(6).equalsIgnoreCase(generate(givenPasswaord));
		}
		else
		if(encryptedPassword.startsWith("{NT}")
			|| encryptedPassword.startsWith("$3$$")
			|| encryptedPassword.startsWith("$NT$"))
		{
			return encryptedPassword.substring(4).equalsIgnoreCase(generate(givenPasswaord));
		}
		return false;
	}

	public static final String crypt(String _prefix, String password)
	{
		return _prefix+generate(password);
	}

	public static final String crypt(String password)
	{
		return BSDNT_PREFIX+generate(password);
	}

	public static final String generate(String password)
	{
		byte[] buf = password.getBytes(StandardCharsets.UTF_16LE);
		return md4Hex(buf).toUpperCase();
	}
	
	public static final byte[] md4(byte[] buf)
	{
		Md4 md = new Md4();
	...
	public static final String md4Hex(byte[] buf)
	{
		return HashUtil.toHex(md4(buf));
	}
