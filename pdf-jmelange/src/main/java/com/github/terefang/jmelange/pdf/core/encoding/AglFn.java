...
public class AglFn
{
	...
	static {
		try {
			...
			AglFn.parseUniFN(AglFn.UNIFN, "afm/aglfn.txt");
	...
	{
		InputStream _rin = ClasspathResourceLoader.of(_file).getInputStream();
	...
	public static void parseUniFN(HashMap<String, Integer> _FN, String _file) throws Exception
	{
		BufferedReader in = new BufferedReader(new InputStreamReader(ClasspathResourceLoader.of(_file).getInputStream()));
		String _line;
		while((_line = in.readLine())!=null)
		{
			if(_line.trim().startsWith("#")) { continue; }
