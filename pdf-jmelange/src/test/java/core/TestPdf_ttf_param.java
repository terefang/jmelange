...
public class TestPdf_ttf_param
{
	...
	public static void main_1(String[] args) throws Exception
	{
		...
		List<PdfFont> _list = _reg.loadTtFonts(PDF.ENCODING_PDFDOC, "./res/fonts");
		for(PdfFont _font : _list)
		{
			...
			_content.drawString("QWERTZUIOP", _POF, _y); _y+=_FSZ+_POF+_POF;
			drawBaseLines(_page, _content, _y, _font);
			_content.drawString("ASDFGHJKLYXCVBNM", _POF, _y); _y+=_FSZ+_POF+_POF;
			drawBaseLines(_page, _content, _y, _font);
			_content.drawString("qwertzuiop", _POF, _y); _y+=_FSZ+_POF+_POF;
			drawBaseLines(_page, _content, _y, _font);
			_content.drawString("asdfghjkl", _POF, _y); _y+=_FSZ+_POF+_POF;
			drawBaseLines(_page, _content, _y, _font);
