...
public class TestPdf_otx_fonts_synth
{
	public static final String[] _fontlist = {
			"/u/fredo/IdeaProjects/jmelange/pdfml-jmelange/target/classes/fonts/urw/urw-fonts/p052003l.pfb.otf",
	...
	public static void main(String[] args) throws Exception
	{
		...
		PdfFontRegistry _reg = PdfFontRegistry.of(doc);

		doc.streamBegin("./out/pdf/test-otx-fonts-synth.pdf");
		...
		_reg.registerFont(_hf, "helv");
		_reg.registerFont(_uni, "uni");
		...
		for(String _fn : _fontlist)
		{
			...
			try
			{
				...
				PdfFont _cf0 = doc.registerOtxFont(PDF.loadFrom(_fn));
				_reg.registerFont(_cf0, _fn);
				PdfFont _cf = _reg.syntheticFont(_fn+"-"+_opt, _fn, PDF.ENCODING_PDFDOC, new String[] {_opt});
				for(int _bmp = 0; _bmp<256; _bmp++)
				{
					if(_cf.hasCoverage(_bmp))
					{
						...
						_content.drawString(new File(_fn).getName()+"-"+_opt, 30, 810);
