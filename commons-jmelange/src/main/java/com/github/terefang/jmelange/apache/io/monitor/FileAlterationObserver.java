 ...
 */
package com.github.terefang.jmelange.apache.io.monitor;
...
import com.github.terefang.jmelange.apache.io.comparator.NameFileComparator;
import com.github.terefang.jmelange.apache.io.filefilter.TrueFileFilter;
import com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener;
import com.github.terefang.jmelange.apache.io.monitor.FileAlterationMonitor;
import com.github.terefang.jmelange.apache.io.monitor.FileEntry;
 ...
 * </p>
 * <ul>
 * <li>Create {@link com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener} implementation(s) that process the file/directory create, change and delete events</li>
 * <li>Register the listener(s) with a {@link FileAlterationObserver} for the appropriate directory.</li>
 * <li>Either register the observer(s) with a {@link com.github.terefang.jmelange.apache.io.monitor.FileAlterationMonitor} or run manually.</li>
 * </ul>
 ...
 * </pre>
 * <p>
 * Alternatively, register the observer(s) with a {@link com.github.terefang.jmelange.apache.io.monitor.FileAlterationMonitor}, which creates a new thread, invoking the observer at the specified interval:
 * </p>
 * <pre>
 *      long interval = ...
 ...
 * <h2>FileEntry</h2>
 * <p>
 * {@link com.github.terefang.jmelange.apache.io.monitor.FileEntry} represents the state of a file or directory, capturing {@link File} attributes at a point in time. Custom
 * implementations of {@link com.github.terefang.jmelange.apache.io.monitor.FileEntry} can be used to capture additional properties that the basic implementation does not support. The
 * {@link com.github.terefang.jmelange.apache.io.monitor.FileEntry#refresh(File)} method is used to determine if a file or directory has changed since the last check and stores the current state of the
 ...
 * </p>
 *
 * @see com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener
...
public class FileAlterationObserver implements Serializable {
     ...
     * List of listeners.
     */
    private transient final List<com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener> listeners = new CopyOnWriteArrayList<>();

    /**
     * The root directory to observe.
     */
    private final com.github.terefang.jmelange.apache.io.monitor.FileEntry rootEntry;
    ...
    public FileAlterationObserver(final File directory, final FileFilter fileFilter, final IOCase ioCase) {
        this(new com.github.terefang.jmelange.apache.io.monitor.FileEntry(directory), fileFilter, ioCase);
    }
     ...
     */
    private FileAlterationObserver(final com.github.terefang.jmelange.apache.io.monitor.FileEntry rootEntry, final FileFilter fileFilter, final Comparator<File> comparator) {
     ...
     * @param ioCase     what case sensitivity to use comparing file names, null means system sensitive.
     */
    protected FileAlterationObserver(final com.github.terefang.jmelange.apache.io.monitor.FileEntry rootEntry, final FileFilter fileFilter, final IOCase ioCase) {
     ...
     */
    public void addListener(final com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener listener) {
        if (listener != null) {
     ...
     */
    private void checkAndFire(final com.github.terefang.jmelange.apache.io.monitor.FileEntry parentEntry, final com.github.terefang.jmelange.apache.io.monitor.FileEntry[] previousEntries, final File[] currentEntries) {
        int                                             c             = 0;
        final com.github.terefang.jmelange.apache.io.monitor.FileEntry[] actualEntries = currentEntries.length > 0 ? new com.github.terefang.jmelange.apache.io.monitor.FileEntry[currentEntries.length] : com.github.terefang.jmelange.apache.io.monitor.FileEntry.EMPTY_FILE_ENTRY_ARRAY;
        for (final com.github.terefang.jmelange.apache.io.monitor.FileEntry previousEntry : previousEntries) {
     ...
     */
    private com.github.terefang.jmelange.apache.io.monitor.FileEntry createFileEntry(final com.github.terefang.jmelange.apache.io.monitor.FileEntry parent, final File file) {
        final com.github.terefang.jmelange.apache.io.monitor.FileEntry entry = parent.newChildInstance(file);
        entry.refresh(file);
     ...
     */
    private void fireOnChange(final com.github.terefang.jmelange.apache.io.monitor.FileEntry entry, final File file) {
     ...
     */
    private void fireOnCreate(final com.github.terefang.jmelange.apache.io.monitor.FileEntry entry) {
        listeners.forEach(listener -> {
            if (entry.isDirectory()) {
     ...
     * @param entry The file entry.
     */
    private void fireOnDelete(final com.github.terefang.jmelange.apache.io.monitor.FileEntry entry) {
        listeners.forEach(listener -> {
            if (entry.isDirectory()) {
     ...
     */
    public Iterable<com.github.terefang.jmelange.apache.io.monitor.FileAlterationListener> getListeners() {
     ...
     */
    private com.github.terefang.jmelange.apache.io.monitor.FileEntry[] listFileEntries(final File file, final com.github.terefang.jmelange.apache.io.monitor.FileEntry entry) {
