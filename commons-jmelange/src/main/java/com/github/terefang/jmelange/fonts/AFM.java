...
public class AFM {
	...
	static {
		try {
			...
			for (int i = 0; i < AFMnames.length; i++) {
				System.err.println(AFMnames[i]);
				AFM afm = new AFM("cp:/afm/" + AFMnames[i]);
	...
	@SneakyThrows
	public static AFM of(File _file)
	{
		return new AFM(_file);
	}
	
	@SneakyThrows
	public static AFM of(InputStream _file)
	{
		return new AFM(_file);
	}
	
	@SneakyThrows
	public static AFM of(ResourceLoader _file)
	{
		return new AFM(_file);
	}
	...
	public static synchronized void parseAglFN()
	{
