...
public final class CUID implements Serializable, Comparable<CUID> {
    ...
    public static CUID randomCUID2(final int length) {
        ...
        if (length <= 0) {
            throw new IllegalArgumentException("the length must be at least 1");
        }
     ...
     *
     * @return byte[] containing the {@code CUID}
     */
    public byte[] toBytes()
    {
        return new BigInteger(this.value,36).toByteArray();
    }
    ...
    public int hashCode() {
        ...
        return value.hashCode()^0x01234567;
    }
