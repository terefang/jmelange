...
public class DefaultEmitter implements Emitter
{
    ...
    @Override
    public void setUseExtensions(boolean _useExtensions)
    {
    ...
    public void emit(final StringBuilder out, final Block root)
    {
        ...
        case HEADLINE:
            ...
            else if (root.hasLines())
            {
                StringBuilder _sb = new StringBuilder();
                this.emitLines(_sb,root);
                out.append(" id=\"");
                out.append(_sb.toString().toLowerCase().replaceAll("[^a-z0-9]+", " ").trim().replace(' ', '-'));
                out.append('"');
            }
            out.append('>');
            break;
