...
public class AsciiPanel extends JPanel {
    ...
    public int getCharScale()
    {
        return charScale;
    }
    
    public void setCharScale(int _charScale)
    {
    ...
    public void paint(Graphics g) {
        ...
        for (int x = 0; x < widthInCharacters; x++) {
            for (int y = 0; y < heightInCharacters; y++) {
                ...
                BufferedImage _glyph = getAsciiFont().getGlyph(newCharacterData.character);
                if(_glyph!=null)
                {
                    if(true)
                    {
                        //LookupOp op = setColors(newCharacterData.backgroundColor, newCharacterData.foregroundColor);
                        //BufferedImage img = op.filter(_glyph, null);
                        ((Graphics2D)offscreenGraphics).setComposite(new Composite()
                        {
                            @Override
                                ...
                                return new CompositeContext()
                                {
                                    ...
                                    public void compose(Raster src, Raster dstIn, WritableRaster dstOut)
                                    {
                                        ...
                                        for (int y = 0; y < h; ++y) {
                                            for (int x = 0; x < w; ++x)
                                            {
                                                _argb = src.getPixel(x, y, _argb);
                                                double _f = ((double)((_argb[0]/*>>>24*/)&0xff))/255.;
                                                Color _c = ColorUtil.rgbLerp(newCharacterData.backgroundColor, newCharacterData.foregroundColor, _f);
                                                _argb[0] = _c.getRed();
                                                _argb[1] = _c.getGreen();
                                                _argb[2] = _c.getBlue();
                                                dstOut.setPixel(x, y, _argb);
                                            }
                                        }
                                    }
                                };
                        ...
                        offscreenGraphics.drawImage(_glyph, x * charWidth, y * charHeight, null);
                    }
                    else
                    {
                        LookupOp op = setColors(newCharacterData.backgroundColor, newCharacterData.foregroundColor);
        ...
        g.drawImage(offscreenBuffer,0,0, offscreenBuffer.getWidth(this)*this.charScale,offscreenBuffer.getHeight(this)*this.charScale,0,0, offscreenBuffer.getWidth(this),offscreenBuffer.getHeight(this),this);
    }
    ...
    {
        if(asciiFont.getGlyph(characterData.character)==null)
        {
            return this;
        }
