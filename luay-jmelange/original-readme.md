...
<h2>Goals of Luaj</h2>
Luaj is a luay.luay.main.lua interpreter based on the 5.2.x version of luay.luay.main.lua with the following goals in mind:
<ul>
<li>Java-centric implementation of luay.lua vm built to leverage standard Java features.
<li>Lightweight, high performance execution of luay.lua. 
...
<h3>Luaj 3.0.x</h3>
Support for luay.luay.main.lua 5.2.x features:
<ul>
...
<h3>Luaj 2.0.x</h3>
Support for luay.luay.main.lua 5.1.x features, plus:
<ul>
<li>Support for compiling luay.lua source code into Java source code.
<li>Support for compiling luay.lua bytecode directly into Java bytecode.
...
<h3>Luaj 1.0.x</h3>
Support for most luay.luay.main.lua 5.1.x features.
...
<tr valign="top">
	...
	<td>11.274</td>
	<td>Java</td>
	<td>java -cp luaj-jse-3.0.2.jar;bcel-5.2.jar luay.luay.main.lua <b>-b</b> fannkuch.luay.luay.main.lua 10</td></tr>
<tr valign="top">
	<td></td>
	<td></td>
	<td>-n (interpreted)</td>
	...
	<td>15.163</td>
	<td></td>
	<td>java -cp luaj-jse-3.0.2.jar luay.lua -n fannkuch.luay.lua 10</td></tr>
<tr valign="top">
	<td>luay.lua</td>
	<td>5.1.4</td>
	<td></td>
	...
	<td>5.477</td>
	<td>C</td>
	<td>luay.lua fannkuch.luay.lua 10</td></tr>
<tr valign="top">
	<td>jill</td>
	<td>1.0.1</td>
	<td></td>
...
the luay.luay.main.lua-to-java-bytecode (luay.luay.main.luajc) compiler is used,
and actually executes <em>faster</em> than C-based luay.luay.main.lua in some cases.  
It is also faster than Java-luay.luay.main.lua implementations Jill, Kahlua, and Mochalua for all benchmarks tested.

<h1>2 - <a name="2">Examples</a></h1>

<h2>Run a luay.luay.main.lua script in Java SE</h2>

<p>
From the main distribution directory line type:

<pre>
	java -cp luaj-jse-3.0.2.jar luay.lua examples/luay.lua/hello.luay.lua
</pre>
...
<pre>
	java -cp luaj-jse-3.0.2.jar luay.lua examples/luay.lua/swingapp.luay.lua
</pre>

<p>
Links to sources:<pre>
	<a href="examples/luay.lua/hello.luay.lua">examples/luay.lua/hello.luay.lua</a>
	<a href="examples/luay.lua/swingapp.luay.lua">examples/luay.lua/swingapp.luay.lua</a>
</pre>

<h2>Compile luay.luay.main.lua source to luay.luay.main.lua bytecode</h2>

<p>
From the main distribution directory line type:

<pre>
	java -cp luaj-jse-3.0.2.jar luay.luac examples/luay.lua/hello.luay.lua
	java -cp luaj-jse-3.0.2.jar luay.lua luay.luac.out
</pre>

<p>
The compiled output "luay.luac.out" is luay.lua bytecode and should run and produce the same result.


<h2>Compile luay.luay.main.lua source or bytecode to java bytecode</h2>

<p>
Luaj can compile luay.lua sources or binaries directly to java bytecode if the bcel library is on the class path. From the main distribution directory line type:

<pre>
	ant bcel-lib
	java -cp &quot;luaj-jse-3.0.2.jar;lib/bcel-5.2.jar&quot; luay.luajc -s examples/luay.lua -d . hello.luay.lua
	java -cp &quot;luaj-jse-3.0.2.jar;.&quot; luay.lua -l hello
</pre>
...
but the compiled classes must be in the class path at runtime, unless runtime jit-compiling via luay.luay.main.luajc and bcel are desired (see later sections).

<p>
Lua scripts can also be run directly in this mode without precompiling using the <em>luay.lua</em> command with the <b><em>-b</em></b> option and providing the <em>bcel</em> library in the class path:
<pre>
	java -cp &quot;luaj-jse-3.0.2.jar;lib/bcel-5.2.jar&quot; luay.lua -b examples/luay.lua/hello.luay.lua
</pre>
...
<pre>
	LuaValue chunk = globals.loadfile("examples/luay.lua/hello.luay.lua");
</pre>

Chunks can also be loaded from a <code>Reader</code> as text source

<pre>
	chunk = globals.load(new StringReader("print 'hello, world'"), "main.luay.lua");
</pre>

or an InputStream to be loaded as text source "t", or binary luay.luay.main.lua file "b":

<pre>
	chunk = globals.load(new FileInputSStream("examples/luay.lua/hello.luay.lua"), "main.luay.lua", "bt"));
</pre>
...
You can also look up the engine by language "luay.luay.main.lua" or mimetypes "text/luay.luay.main.lua" or "application/luay.luay.main.lua".
...
<h2>Excluding the luay.luay.main.lua bytecode compiler</h2>
...
The "luay.luay.main.luac" utility is provided in the jse jar for this purpose, or a standard luay.luay.main.lua compiler can be used.

<p>
To exclude the luay.luay.main.lua-to-luay.luay.main.lua-bytecode compiler, do not call 
...
<pre>
	luay.vm.compiler.LuaC.install(globals);
</pre>


<h2>Including the LuaJC luay.luay.main.lua-bytecode-to-Java-bytecode compiler</h2>

<p>
To compile from luay.luay.main.lua to Java bytecode for all luay.luay.main.lua loaded at runtime, 
...
<p>
This will compile all luay.lua bytecode into Java bytecode, regardless of if they are loaded as
luay.lua source or luay.lua binary files.
...
<li>Metatables for Number, String, Thread, Function, Boolean, and and Nil 
 are shared and therefore should not be mutated once luay.lua code is running in any thread.
</ul>
...
<li><a href="examples/luay.lua/samplesandboxed.luay.lua">examples/jse/samplesandboxed.luay.lua</a>
A luay.luay.main.lua sandbox that limits librares,limits bytecodes per script, and makes shared tables read-only
...
See <a href="http://www.luay.luay.main.lua.org/manual/5.1/">standard luay.luay.main.lua documentation</a> for details on the library API's
...
as with the yield-from-anywhere patch in C-based luay.luay.main.lua.
...
To install dynamically from luay.luay.main.lua use java-class-based require:</em>:
<pre>
	require 'org.luaj.vm2.lib.DebugLib'
</pre>

The <em>luay.luay.main.lua</em> command line utility includes the <em>debug</em> library by default.
...
<p>
The following luay.lua script will open a swing frame on Java SE:
<pre>
...
<p>
See a longer sample in <em>examples/luay.lua/swingapp.luay.lua</em> for details, including a simple animation loop, rendering graphics, mouse and key handling, and image loading. 
...
<p>
The <em>luay.lua</em> connand line tool includes <em>luajava</em>. 
...
All luay.luay.main.lua value manipulation is now organized around
<a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/LuaValue.html">LuaValue</a>
which exposes the majority of interfaces used for luay.luay.main.lua computation.
<pre>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/LuaValue.html">luay.vm.LuaValue</a>
</pre>
...
<pre>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/Varargs.html">luay.vm.Varargs</a>
</pre>
...
<pre>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/lib/ZeroArgFunction.html">luay.vm.lib.ZeroArgFunction</a>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/lib/OneArgFunction.html">luay.vm.lib.OneArgFunction</a>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/lib/TwoArgFunction.html">luay.vm.lib.TwoArgFunction</a>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/lib/ThreeArgFunction.html">luay.vm.lib.ThreeArgFunction</a>
	 <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/lib/VarArgFunction.html">luay.vm.lib.VarArgFunction</a>
</pre>
...
<p>
Calling this function from luay.lua could be done by:
<pre> 
...
Note that in both the luay.luay.main.lua and Java case, extra arguments will be ignored, and the function will be called.  
...
in the environment that can be called from luay.luay.main.lua. 
...
<p>
The luay.luay.main.lua script used to load and test it is in <a href="examples/luay.lua/hyperbolicapp.luay.lua">examples/luay.lua/hyperbolicapp.luay.lua</a>
<pre>
	require 'hyperbolic'
...
<h2>Closures</h2>
Closures still exist in this framework, but are optional, and are only used to implement luay.luay.main.lua bytecode execution,
and is generally not directly manipulated by the user of luaj.
<p>
See the <a href="http://luaj.org/luaj/3.0/api/org/luaj/vm2/LuaClosure.html">luay.vm.LuaClosure</a> 
javadoc for details on using that class directly. 

<h1>6 - <a name="6">Parser</a></h1>

<h2>Javacc Grammar</h2>
A Javacc grammar was developed to simplify the creation of Java-based parsers for the luay.luay.main.lua language.
...
The default lu compiler does a single-pass compile of luay.luay.main.lua source to luay.luay.main.lua bytecode, so no explicit parse tree is produced.

<p>
To simplify the creation of abstract syntax trees from luay.lua sources, the LuaParser class is generated as part of the JME build.  
...
<pre>
	test/junit/org/luaj/vm2/AllTests.luay.lua
</pre>

<p>
Unit test scripts can be found in these locations
<pre>
	test/luay.lua/*.luay.lua
	test/luay.lua/errors/*.luay.lua
	test/luay.lua/perf/*.luay.lua
	test/luay.lua/luaj3.0.2-tests.zip
</pre>
...
<li>JSR-223 enhancement: allow both ".luay.lua" and "luay.lua" as extensions in getScriptEngine() </li>
...
<li>Fix luay.lua command vararg values passed into main script to match what is in global arg table </li>
...
<li>Convert internal and external API's to match luay.lua 5.2.x environment changes </li>
<li>Add bit32 library </li>
<li>Add explicit Globals object to manage global state, especially to imrpove thread safety </li>
<li>Drop support for luay.lua source to java surce (lua2java) in favor of direct java bytecode output (luay.luajc) </li>
<li>Remove compatibility functions like table.getn(), table.maxn(), table.foreach(), and math.log10() </li>
<li>Add ability to create runnable jar file from luay.lua script with sample build file build-app.xml </li>
...
<li>Add optional -c encoding flag to luay.lua, luay.luac, and luay.luajc tools to control source encoding.</li>
...
<li>Establish "org.luaj.debug" and "luay.test.luajc" system properties to configure scripting engine.</li>
...
<li>Allow error() function to pass any luay.lua object including non-strings.</li>
...
<li>Allow access to Java inner classes using luay.lua field syntax (fixes issue #40).</li>
...
<li>Add samplesandboxed.luay.lua example code to illustrate sandboxing techniques in luay.lua.</li>
<li>Add CollectingOrphanedCoroutines.java example code to show how to deal with orphaned luay.lua threads.</li>
...
<li>luay.lua compiled into java bytecode using luay.luajc cannot use string.dump() or xpcall()
...
<li>command-line tools luay.luay.main.lua, luay.luay.main.luac, and luay.luay.main.luajc will require <em>-c Cp037</em> to specify the encoding
</ul>
...
Code that is generated on the fly within luay.luay.main.lua and compiled with luay.luay.main.lua's <em>load()</em> function
