...
public class SystemContext
{
	...
	public void init()
	{
		...
		double _surface_distance_au = this.context.getSurfaceDistance()*SolarContext.SOL_TO_AU;
		...
		try
		{
			int _num_rad = StarSysUtil.getInt("stellar_type['"+this.context.getSpectra()+"']['"+this.context.getRsize()+"']['orbitals']");
			float _planet_chance = StarSysUtil.getFloat("stellar_type['"+this.context.getSpectra()+"']['"+this.context.getRsize()+"']['planet_chance']");
			for(int _orbital = 0; _orbital <_num_rad; _orbital++)
			{
				...
				PlanetContext _planet = PlanetContext.tabled(_seed^_orbital, this.context, _orbit, _orbital, false);
