...
public class MoonContext {
    ...
    public static List<MoonContext> generate(int _seed, PlanetContext _planet) {
        double _moon_max = StarSysUtil.getFloat("planet_table['" + _planet.type + "']['max_num_moons']");
        double _moon_mass_max = StarSysUtil.getFloat("planet_table['" + _planet.type + "']['max_combined_mass_of_moons']");
        ...
        for (MoonContext _ctx : _list) {
            ...
            if (_moon_radian_last > _planet.getMoonLimitAU()*SolarContext.AU_TO_KM) break;
        }
    ...
    public void outputInformation(Appendable _fh) {
        ...
        _fh.append(String.format("       Moon Orbit: % 12.3f km, %.3f AU (%d)\n", this.getOrbit(), this.getOrbit()/SolarContext.AU_TO_KM, this.getOrbital()));
