...
public class PlanetContext
{
    ...
    public static PlanetContext tabled(int _seed, SolarContext _sol, double _orbit, int _orbital, boolean _gaia)
    {
        ...
        String _lookup = StarSysUtil.getList("planet_table['"+(_gaia?"gaia-":"")+"zones']").get(_ctx.zone-1);
        while(_lookup.startsWith("*"))
        {
            _lookup = StarSysUtil.lookupTabled(_rng, "planet_table", _lookup);
        }

        _ctx.type = _lookup;
        _ctx.description = StarSysUtil.getString("planet_table['"+_lookup+"']['description']");
    ...
    public void randomizeFrom(int _seed, boolean _gaia)
    {
        ...
        double _rot_mul = StarSysUtil.getFloat("planet_table['"+this.type+"']['rotational_multiplier']");
        double _dia_base = StarSysUtil.getFloat("planet_table['"+this.type+"']['diameter_base']");
        double _dia_var = StarSysUtil.getFloat("planet_table['"+this.type+"']['diameter_variation']");
        double _dia_mul = StarSysUtil.getFloat("planet_table['"+this.type+"']['diameter_multiplier']");
        double _dens_base = StarSysUtil.getFloat("planet_table['"+this.type+"']['density_base']");
        double _dens_var = StarSysUtil.getFloat("planet_table['"+this.type+"']['density_variation']");
        double _axial_tilt_max = StarSysUtil.getFloat("planet_table['"+this.type+"']['max_axial_tilt']");

        ArcRand _rng = ArcRand.from(_seed + this.seed + (new BigInteger(1, HashUtil.md5(this.type))).intValue());
        ...
        this.planetYear=Math.sqrt(Math.pow(this.orbit,3)/_sol.getMass());
        ...
        this.molLimit = StarSysUtil.calcMoleculeLimit(this.planetMass*5.9742e24, this.planetDiameter, _sol.getLuminosity());
        ...
        if(this.gasInventory > 1)
        {
            ...
            while(_lookup.startsWith("*"))
            {
                _lookup = StarSysUtil.lookupTabled(_rng, "atmos_table", _lookup);
            }
        }
